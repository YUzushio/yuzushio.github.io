...
### 推奨: `@gallery-setup` で fork → 公開まで

clone 後、**Cursor / VS Code / Claude Code** で **このフォルダ（Cloneしたフォルダ）** をワークスペースとして開き:

| エディタ | 操作 |
|---------|------|
| **Cursor / Claude Code** | `@gallery-setup` と入力し、対話に従う |
| **VS Code + Copilot** | [`.cursor/skills/gallery-setup/SKILL.md`](.cursor/skills/gallery-setup/SKILL.md) を Chat で参照 |
...
`gallery.json` の `meta` 更新 · User Pages のリポ名チェック · ビルド確認 · GitHub Pages デプロイ手順まで案内します。

以下 §1 以降は **手動で進める場合** の詳細です（Skill 実行後は §2 以降を必要に応じて参照）。
...
[`@gallery-setup`](#推奨-gallery-setup-で-fork--公開まで) 済みなら **この節はスキップ可**。手動で編集する場合は [`public/data/gallery.json`](public/data/gallery.json) の `meta` を変更:
